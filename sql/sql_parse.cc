...
mysql_create_routine(THD *thd, LEX *lex)
{
  ...
  auto type= lex->sphead->m_handler->type();
  bool is_synonym= type == SP_TYPE_SYNONYM;
  ...
  if (lex->create_info.or_replace())
  {
    ...
    {
      auto alter_priv= lex->sphead->m_db.streq(any_db) ? CREATE_PUBLIC_SYNONYM_ACL : ALTER_SYNONYM_ACL;
...
absent:
  ...
  auto alter_priv= !is_synonym ? ALTER_PROC_ACL : ALTER_SYNONYM_ACL;
  if (is_synonym && lex->spname->m_db.streq(any_db))
    alter_priv= CREATE_PUBLIC_SYNONYM_ACL;
  if (check_routine_access(thd, alter_priv, &lex->spname->m_db,
                           &lex->spname->m_name,
                           Sp_handler::handler(lex->sql_command), 0))
    return 1;

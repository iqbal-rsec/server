...
bool Type_handler_composite::Item_eq_value(THD *thd, const Type_cmp_attributes *attr,
                                     Item *a, Item *b) const
{
  DBUG_ASSERT(0);
  return false;
...
Item *Type_handler_composite::create_item_method(THD *thd,
                                              ...
                                              List<Item> *args) const
{
  ...
  Item_splocal *array= thd->lex->create_item_for_sp_var(ca, NULL);
  if (unlikely(array == NULL))
    return NULL;

  return create_item_method(thd, array, cb, args);
}

...
bool Sql_path::init()
{
  ...
  //return add_schema(cur_schema.str, cur_schema.length);
  return false;
}
...
bool Sql_path::add_schema(const char *schema_str, size_t schema_len)
{
  ...
  */
  for (size_t i= 0; i < m_count; i++)
  {
    if (m_schemas[i].length == schema_len &&
...
bool Sql_path::from_text(THD *thd, CHARSET_INFO *cs, const LEX_CSTRING &text)
{
  ...
  while (curr != end)
  {
    ...
    if (state == tokenize_state::END)
    {
      ...
      {
        if (add_schema(token_start, (size_t)(token_end - token_start)))
          return true;
